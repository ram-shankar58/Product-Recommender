"""
Generate realistic e-commerce data with authentic user behavior patterns.
This creates a believable product catalog and user interactions.
"""
import csv
import random
...
# Realistic product catalog across different categories
PRODUCTS = [
    # Electronics
...
def generate_interactions():
    """Generate realistic user interactions based on personas."""
    interactions = []
    interaction_id = 1
    ...
    for user in USER_PERSONAS:
        ...
        # Filter products that match user interests
        matching_products = []
        ...
        # Add some random products (browsing behavior)
        all_products = matching_products + random.sample(
            [p for p in PRODUCTS if p not in matching_products], 
            min(5, len(PRODUCTS) - len(matching_products))
        )
        
        # Generate 5-15 interactions per user over 90 days
        ...
        for timestamp in user_timeline:
            # Choose product (higher chance for matching interests)
            ...
            if budget == "low" and product["price"] > 200:
                if random.random() < 0.7:  # 70% chance to skip expensive items
                    continue
            
            # Determine interaction type (realistic funnel)
            rand = random.random()
            if rand < 0.5:
                event = "view"
            elif rand < 0.8:
